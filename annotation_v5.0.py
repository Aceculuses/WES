...
#--------------------------------------------------------------------------------------
#    ***For vcf annotation and transfer it to required format
#    ***The current version is latest version 5
#    ***The control flow has been added to manage multi-subprocess
#    ***Implement Object Orientation Programming

#    ***Author: Guoping Gu
#    ***Email: guoping.gu@outlook.com
#--------------------------------------------------------------------------------------

#create folders
def mkdir(index,pathList,genomeVersion):
...
#
def returnPath(genomeVersion,pathList):
...
#concat files
def concat(path,genomeVersion,cmd_bgzip,cmd_tabix,cmd_bcf,bcfPath):
...
#LiftOver hg19Thg38
def liftover(genomeVersion,inpath,outpath,cmd_liftover):
    pidd=set()
    if genomeVersion == 'hg19':
        pass
    elif genomeVersion == 'hg38':
...
#Convert Format
def convert(path,cmd_convert2annovar):
    pidd=set()
    file=os.listdir(path)
...
#Reformat
def reformat(inpath,cmd_reformat):
    pidd=set()
    file=os.listdir(inpath)
    for filename in file:
...
#
def transfer(path,cmd_transfer,cmd_rm):
    pidd=set()
    file=os.listdir(path)
    for filename in file:
...
def checkClinAvail(genomeVersion,clinDate):
